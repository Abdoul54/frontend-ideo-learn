...
import { Container, Box, Skeleton, Typography, Divider, useMediaQuery, Chip, Card, CardHeader, CardContent } from '@mui/material';
...
import Footer from '@/components/learner/components/Footer';
import OptionMenu from '@/@core/components/option-menu';
import { useBanners, useFooter, useNews, useSliders, useWelcomeDefault, useWidgetSettings } from '@/hooks/api/tenant/widgets/useWidgets';
...
export default function LearnerHomePage() {
  ...
  // Are any right-side widgets enabled? (chart or banners)
  ...
  const renderChartCard = () => {
    if (hasMount && isUserReportEnabled) {
      return (
        <Card sx={{ mb: 5, pb: 2, height: '250px', borderRadius: 2 }}>
          <CardHeader
            title='Course Activity Stats'
            titleTypographyProps={{ variant: 'h6' }}
            action={<OptionMenu options={['Refresh', 'Export']} />}
          />
          <CardContent>
              <AppReactApexCharts
                type='donut'
      ...
      <Box sx={{ pb: 8 }}>
        <Container maxWidth="xl">
          ...
          <Grid container spacing={4} sx={{ mt: 4 }}>
            {/* Left Column - Courses */}
            <Grid item size={{ xs: 12, md: hasRightSideWidgets ? 8 : 8 }}>
              {renderCourses()}
            </Grid>
